...
const enhancePrompt = (prompt: string): string => {
  // First replace any uppercase version with lowercase
  const normalizedPrompt = prompt.replace(new RegExp(TRIGGER, 'i'), TRIGGER)

  // Then check if the trigger exists
  if (!normalizedPrompt.includes(TRIGGER)) {
    return `${TRIGGER} style, ${normalizedPrompt.trim()}`
  }
...
export async function POST(req: Request) {
  if (!process.env.REPLICATE_API_TOKEN) {
    throw new Error('The REPLICATE_API_TOKEN environment variable is not set. See README.md')
  }
  ...
  pp(input, 'INPUT DATA...CREATING PREDICTION...')
  ...
  try {
    ...
    return new Response(JSON.stringify({ detail: 'Failed to generate image' }), { status: 500 })
  }
}
